...
exports.default = exports.APNS = void 0;
var _nodeApn = _interopRequireDefault(require("@parse/node-apn"));
...
class APNS {
  ...
  constructor(args) {
    ...
    if (Array.isArray(args)) {
      ...
      throw new _parse.default.Error(_parse.default.Error.PUSH_MISCONFIGURED, 'APNS Configuration is invalid');
    }
    ...
    for (let apnsArgs of apnsArgsList) {
      ...
      if (apnsArgs.bundleId) {
        _npmlog.default.warn(LOG_PREFIX, 'bundleId is deprecated, use topic instead');
  ...
  send(data, allDevices) {
    ...
    if (!coreData || !allDevices || !Array.isArray(allDevices)) {
      _npmlog.default.warn(LOG_PREFIX, 'invalid push payload');
      return;
    }
  ...
  static _createProvider(apnsArgs) {
    ...
    if (!APNS._validateAPNArgs(apnsArgs)) {
      throw new _parse.default.Error(_parse.default.Error.PUSH_MISCONFIGURED, 'topic is mssing for %j', apnsArgs);
    }
    let provider = new _nodeApn.default.Provider(apnsArgs);
  ...
  static _generateNotification(coreData, headers) {
    let notification = new _nodeApn.default.Notification();
    let payload = {};
  ...
  _handlePromise(response) {
    ...
    response.sent.forEach(token => {
      _npmlog.default.verbose(LOG_PREFIX, 'APNS transmitted to %s', token.device);
  ...
  static _handlePushFailure(failure) {
    if (failure.error) {
      _npmlog.default.error(LOG_PREFIX, 'APNS error transmitting to device %s with error %s', failure.device, failure.error);
      ...
      _npmlog.default.error(LOG_PREFIX, 'APNS error transmitting to device %s with status %s and reason %s', failure.device, failure.status, failure.response.reason);
      return APNS._createErrorPromise(failure.device, failure.response.reason);
    } else {
      _npmlog.default.error(LOG_PREFIX, 'APNS error transmitting to device with unkown error');
...
exports.APNS = APNS;
var _default = exports.default = APNS;