...
function FCM(args) {
  if (typeof args !== 'object' || !args.firebaseServiceAccount) {
    throw new _parse.default.Error(_parse.default.Error.PUSH_MISCONFIGURED, 'FCM Configuration is invalid');
  }
  let app;
  if ((0, _app.getApps)().length === 0) {
    app = (0, _app.initializeApp)({
      credential: (0, _app.cert)(args.firebaseServiceAccount)
    });
  } else {
    app = (0, _app.getApp)();
  }
...
FCM.prototype.send = function (data, devices) {
  ...
  const sendToDeviceSlice = deviceSlice => {
    const pushId = (0, _PushAdapterUtils.randomString)(10);
    ...
    _npmlog.default.info(LOG_PREFIX, `sending push to ${length} devices`);
    return this.sender.sendEachForMulticast(fcmPayload.data).then(response => {
      ...
      response.responses.forEach((resp, idx) => {
        if (resp.success) {
          ...
          _npmlog.default.error(LOG_PREFIX, `failed to send to ${deviceTokens[idx]} with error: ${JSON.stringify(resp.error)}`);
        }
      });
      if (failedTokens.length) {
        _npmlog.default.error(LOG_PREFIX, `tokens with failed pushes: ${JSON.stringify(failedTokens)}`);
      }
      if (successfulTokens.length) {
        _npmlog.default.verbose(LOG_PREFIX, `tokens with successful pushes: ${JSON.stringify(successfulTokens)}`);
      }
      return Promise.all(promises);
    });
  };
  const allPromises = Promise.all(slices.map(sendToDeviceSlice)).catch(err => {
    _npmlog.default.error(LOG_PREFIX, `error sending push: ${err}`);
  });
  return allPromises;
};
