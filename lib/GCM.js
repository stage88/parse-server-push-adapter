...
var _nodeGcm = _interopRequireDefault(require("@parse/node-gcm"));
...
function GCM(args) {
  if (typeof args !== 'object' || !args.apiKey) {
    throw new _parse.default.Error(_parse.default.Error.PUSH_MISCONFIGURED, 'GCM Configuration is invalid');
  }
  this.sender = new _nodeGcm.default.Sender(args.apiKey, args.requestOptions);
}
...
GCM.prototype.send = function (data, devices) {
  ...
  let pushId = (0, _PushAdapterUtils.randomString)(10);
  // Make a new array
  ...
  if (slices.length > 1) {
    _npmlog.default.verbose(LOG_PREFIX, `the number of devices exceeds ${GCMRegistrationTokensMax}`);
  ...
  let message = new _nodeGcm.default.Message(gcmPayload);
  ...
  _npmlog.default.verbose(LOG_PREFIX, `sending to ${length} ${length > 1 ? 'devices' : 'device'}`);
  this.sender.send(message, {
    ...
    */
    if (error) {
      _npmlog.default.error(LOG_PREFIX, `send errored: %s`, JSON.stringify(error, null, 4));
    } else {
      _npmlog.default.verbose(LOG_PREFIX, `GCM Response: %s`, JSON.stringify(response, null, 4));
    }
    let {
      results,
      multicast_id
